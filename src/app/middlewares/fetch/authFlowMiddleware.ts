  ...
  (store) => (next) => async (action: unknown) => {
    if ((action as Action).type === loginSlice.fulfilled.type) {
      try {
        ...
        //   // Step 4: Redirect to the dashboard after the data is loaded
        //   console.log('Products fetched successfully, redirecting to dashboard...');
        //   Router.push('/dashboard');
        //   console.log('Redirect should be happening now');
      } catch (error: unknown) {
